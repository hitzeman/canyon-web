 ...
 *
 * Runs after `ng build` (see the `build` script). It imports the same data
 * modules the app uses, so the sitemap can't drift from the prerendered routes.
 * Node executes this file directly, stripping the TypeScript types.
 */
import { copyFile, writeFile } from 'node:fs/promises';
import { join } from 'node:path';

import { PAGES } from '../src/app/core/pages.ts';
import { getProducts } from '../src/app/core/products.ts';
import { newsletter, site } from '../src/app/core/site.ts';
...
if (!newsletter.action) {
  console.warn(
    'postbuild: warning: newsletter.action is not set in src/app/core/site.ts, so the sign-up form ships disabled.',
  );
}
